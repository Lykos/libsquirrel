...
namespace DataStructures {
  ...
  namespace LongArithmetic {
    ...
    std::pair<const LongInt::part_type*, const LongInt::part_type*> inline calc_xy2(const LongInt::part_type* xy0_begin,
                                                                        const LongInt::part_type* xy0_end,
                                                                        const LongInt::part_type* xy1_begin,
                                                                        const LongInt::part_type* xy1_end,
                                                                        ...
                                                                        LongInt::part_type* c_end)
    {
      if (xy1_begin < xy1_end) {
        ...
        LongInt::part_type* xy2_begin (c_begin);
        LongInt::part_type* xy2_end (c_begin + part_size);
                                 ...
                                 LongInt::part_type* c_end)
    {
      ...
      if (a_size <= 0 || b_size <= 0) {
        ...
        LongInt::part_type *res_begin (c_begin), *res_end (c_begin);
        *res_begin = c0;
        ++res_end;
        if (c1 > 0) {
          *res_end = c1;
      ...
      const LongInt::part_type* x0_begin (a_begin);
      const LongInt::part_type* x0_end (std::min(a_begin + part_size, a_end));
      const LongInt::part_type* x1_begin (x0_end);
      const LongInt::part_type* x1_end (a_end);
      const LongInt::part_type* y0_begin (b_begin);
      const LongInt::part_type* y0_end (std::min(b_begin + part_size, b_end));
      const LongInt::part_type* y1_begin (y0_end);
      const LongInt::part_type* y1_end (b_end);
      LongInt::part_type* res_begin (c_begin);

      // z0 = x0 * y0
      LongInt::part_type* z0_begin (res_begin); // Is the first part of the actual result
      LongInt::part_type* z0_end = multiply(x0_begin, x0_end, y0_begin, y0_end, z0_begin, c_end); // z0 = x0 * y0
      // This has to be padded to size at least 2 * part_size because the final result is constructed here
      for (LongInt::part_type* z0_it = z0_end; z0_it < z0_begin + 2 * part_size; ++z0_it) {
        *z0_it = 0l;
      }

      // z2 = x1 * y1
      LongInt::part_type* z2_begin (std::max(z0_end, z0_begin + 2 * part_size)); // Is the second part of the actual result
      LongInt::part_type* z2_end = multiply(x1_begin, x1_end, y1_begin, y1_end, z2_begin, c_end);
      LongInt::part_type* res_end (z2_begin + part_size);
      if (z2_begin < z2_end) {
        res_end += part_size;
      }
      ...
      for (LongInt::part_type* z2_it = z2_end; z2_it < res_end; ++z2_it) {
        *z2_it = 0l;
      }
      LongInt::part_type* z1_begin (res_end);

      // x2 = x0 + x1
      std::pair<const LongInt::part_type*, const LongInt::part_type*> x2 = calc_xy2(x0_begin, x0_end, x1_begin, x1_end, z1_begin, c_end);
      const LongInt::part_type *x2_begin (x2.first), *x2_end (x2.second);

      // y2 = y0 + y1
      std::pair<const LongInt::part_type*, const LongInt::part_type*> y2 = calc_xy2(y0_begin, y0_end, y1_begin, y1_end, z1_begin, c_end);
      const LongInt::part_type *y2_begin (y2.first), *y2_end (y2.second);

      // z1 = x2 * y2 - z0 - z2
      LongInt::part_type *z1_end = multiply(x2_begin, x2_end, y2_begin, y2_end, z1_begin, c_end);
